...
export CFLAGS="-fPIC -m32 $CFLAGS"
export CXXFLAGS="-fPIC -m32 $CXXFLAGS"
...
cd sources/fdk-aac
./autogen.sh
linux32 ./configure --prefix="${INSTALL_PREFIX}" --disable-shared --enable-static
make clean
make -j 6 VERBOSE=1
make install VERBOSE=1
cd ../..
...
cd sources/frei0r
./autogen.sh
linux32 ./configure --prefix="${INSTALL_PREFIX}" --enable-static --disable-shared
make clean
make -j 6 VERBOSE=1
make install VERBOSE=1
cd ../..
...
cd sources/fribidi
./bootstrap
linux32 ./configure --prefix="${INSTALL_PREFIX}" --disable-shared --enable-static --disable-debug --disable-deprecated
make clean
make -j 6 VERBOSE=1
make install VERBOSE=1
cd ../..
...
linux32 ./configure --prefix="${INSTALL_PREFIX}" --disable-shared --enable-static --enable-nasm CFLAGS="-msse4.1"
make clean
make -j 6 VERBOSE=1
make install VERBOSE=1
cd ../..
...
cd sources/libass
./autogen.sh
linux32 ./configure --prefix="${INSTALL_PREFIX}" --disable-shared --enable-static --disable-require-system-font-provider
make clean
make -j 6 VERBOSE=1
make install VERBOSE=1
cd ../..
...
./autogen.sh
linux32 ./configure --prefix="${INSTALL_PREFIX}" --disable-shared --enable-static --disable-sqlite --disable-external-libs --disable-full-suite
make clean
make -j 6 VERBOSE=1
make install VERBOSE=1
...
./autogen.sh
linux32 ./configure --prefix="${INSTALL_PREFIX}" --disable-shared --enable-static --disable-docs --disable-examples --disable-oggtest
make clean
make -j 6 VERBOSE=1
make install VERBOSE=1
cd ../..
...
cd sources/libvpx
linux32 ./configure --prefix="${INSTALL_PREFIX}" --target=x86-linux-gcc --disable-shared --enable-static --disable-examples --disable-tools --disable-docs --disable-unit-tests --enable-vp9-highbitdepth --enable-pic
make clean
make -j 6 VERBOSE=1
make install VERBOSE=1
cd ../..
...
linux32 cmake -G"Unix Makefiles" . -DCMAKE_INSTALL_PREFIX="${INSTALL_PREFIX}" -DBUILD_SHARED_LIBS=0 -DBUILD_CODEC=0 -DCMAKE_C_FLAGS="$CFLAGS" -DCMAKE_CXX_FLAGS="$CXXFLAGS"
make clean
make -j 6 VERBOSE=1
make install VERBOSE=1
cd ../..
...
linux32 ./configure --prefix="${INSTALL_PREFIX}" --disable-shared --enable-static --disable-doc --disable-extra-programs
make clean
make -j 6 VERBOSE=1
make install VERBOSE=1
cd ../..
...
find . -name "CMakeFiles" |xargs rm -rf
find . -name "CMakeCache.txt" |xargs rm -rf
linux32 cmake -G"Unix Makefiles" . -DCMAKE_INSTALL_PREFIX="${INSTALL_PREFIX}" -DBUILD_SHARED_LIBS=0 -DHAVE_WORDS_BIGENDIAN_EXITCODE=0 -DWITH_OPENMP=0 -DBUILD_TESTS=0 -DBUILD_EXAMPLES=0 -DCMAKE_C_FLAGS="$CFLAGS -msse4.1" -DCMAKE_CXX_FLAGS="$CXXFLAGS -msse4.1"
make clean
make -j 6 VERBOSE=1
make install VERBOSE=1
cd ../..
...
cd sources/x264
linux32 ./configure --prefix="${INSTALL_PREFIX}" --disable-shared --enable-static --disable-cli --enable-strip --enable-pic
make clean
make -j 6 VERBOSE=1
make install VERBOSE=1
cd ../..
...
echo -e "\033[32mbuild linux32 done with ${build_duration_minutes}m:${build_duration_seconds}s.\033[0m"