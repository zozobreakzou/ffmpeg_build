...
# mv /usr/i686-w64-mingw32/lib/libpthread.dll.a       /usr/i686-w64-mingw32/lib/hide_libpthread.dll.a
# mv /usr/i686-w64-mingw32/lib/libwinpthread.dll.a    /usr/i686-w64-mingw32/lib/hide_libwinpthread.dll.a
#
...
./configure --host="$TARGET_HOST" --prefix="${INSTALL_PREFIX}" --disable-shared --enable-static --enable-nasm  CFLAGS="-msse4.1"
make clean
make -j 6 VERBOSE=1
make install VERBOSE=1
cd ../..
...
cd sources/libvpx
./configure --target="x86-win32-gcc" --prefix="${INSTALL_PREFIX}" --disable-shared --enable-static --disable-examples --disable-tools --disable-docs --disable-unit-tests --enable-vp9-highbitdepth
make clean
make -j 6 VERBOSE=1
make install VERBOSE=1
cd ../..
...
cmake -G"Unix Makefiles" . -DCMAKE_INSTALL_PREFIX="${INSTALL_PREFIX}" -DBUILD_SHARED_LIBS=0 -DBUILD_CODEC=0 -DCMAKE_SYSTEM_NAME=Windows -DCMAKE_TOOLCHAIN_FILE=../../toolchain-mingw32.cmake
make clean
make -j 6 VERBOSE=1
make install VERBOSE=1
cd ../..
...
find . -name "CMakeFiles" |xargs rm -rf
find . -name "CMakeCache.txt" |xargs rm -rf
cmake -G"Unix Makefiles" . -DCMAKE_INSTALL_PREFIX="${INSTALL_PREFIX}" -DBUILD_SHARED_LIBS=0 -DHAVE_WORDS_BIGENDIAN_EXITCODE=0 -DWITH_OPENMP=0 -DBUILD_TESTS=0 -DBUILD_EXAMPLES=0 -DCMAKE_SYSTEM_NAME=Windows -DCMAKE_TOOLCHAIN_FILE=../../toolchain-mingw32.cmake -DCMAKE_C_FLAGS="-msse4.1" -DCMAKE_CXX_FLAGS="-msse4.1"
make clean
make -j 6 VERBOSE=1
make install VERBOSE=1
cd ../..
...
--extra-cflags="-I${INSTALL_PREFIX}/include -static-libgcc" \
--extra-ldflags="-L${INSTALL_PREFIX}/lib -static-libgcc" \
--disable-w32threads \
--disable-debug \
--enable-gpl \
--enable-version3 \
...
echo -e "\033[32mbuild win32 done with ${build_duration_minutes}m:${build_duration_seconds}s.\033[0m"